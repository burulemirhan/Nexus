...
import { useMiraLanguage } from '../contexts/LanguageContext';

interface HeroPageProps {
  onCropSelect: (crop: CropConfig) => void;
}

const NEXUS_URL = import.meta.env.MODE === 'development' 
  ? 'http://localhost:3001' 
  : 'https://nexusbiotech.org';

export default function HeroPage({ onCropSelect }: HeroPageProps) {
  const { t, language, setLanguage } = useMiraLanguage();
