  ...
  return (
    <nav 
      className={`fixed top-2 md:top-3 left-0 w-full z-50 transition-opacity duration-300 ease-out ${
        isVisible && !isMenuOpen ? 'opacity-100' : isMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'
      }`}
    ...
    >
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className={`md:hidden fixed inset-0 ${darkMode ? 'bg-white/95 backdrop-blur-md' : 'bg-nexus-dark/95 backdrop-blur-md'} p-6 flex flex-col justify-center gap-4 text-lg font-display font-bold tracking-widest uppercase z-[60] ${darkMode ? 'text-black' : 'text-white'}`} style={{ top: 0, left: 0, right: 0, bottom: 0 }}>
           <button 
             ...
             {isLangOpen && (
               <div className={`mt-2 ${darkMode ? 'bg-white/95 backdrop-blur-md border border-black/20' : 'bg-black/85 backdrop-blur-md border border-white/10'} rounded-md overflow-hidden`}>
                 <button
                   ...
                   className={`w-full px-4 py-3 text-left text-base font-display font-bold tracking-widest uppercase transition-colors touch-manipulation ${
                     language === 'tr'
                      ? darkMode ? 'text-black bg-black/10' : 'text-white bg-white/10'
                       : darkMode ? 'text-black/70 hover:text-black hover:bg-black/5' : 'text-white/70 hover:text-white hover:bg-white/5'
                   }`}
